...
@Component({
  ...
  styleUrls: ['./completedorder.component.css']
})
export class CompletedorderComponent implements OnInit {
  ...
  totalorders: any
  getcarddetails() {
    this.coreService.getcarditems().subscribe((res) => {
      this.totalorders = res;
      if (this.isuseractesasadmin === true) {
        this.orders = this.totalorders.filter((order: any) =>
          order['0'] && order['0'][0] && order['0'][0].adminid === this.adminId
        );
       
        this.sortByDateDescending();
        this.orders=this.filterByCompletedOrder()
      } else {
        this.orders = this.totalorders.filter((order: any) =>
  ...
  filterByCompletedOrder() {
    const pendingOrders = this.orders.filter((order: string[]) => order[4] === 'Completed');
  ...
  onChange(): void {
    ...
    this.coreService.getCardItemsFilteredByDate(searchdate).subscribe({
      next: (data) => {
  ...
  seachcard() {
    this.coreService.getCardItemsFilteredByinvoice(this.searchvaluecard).subscribe({
      next: (data) => {
  ...
  async deletetableitem(id: any) {
    await this.coreService.deletecarditems(id).subscribe((res) => {
      console.log(res);
    });
